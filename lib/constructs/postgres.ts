...
export class Postgres extends Construct implements IConnectable {
  ...
  constructor(scope: Construct, id: string, props: PostgresProps) {
    ...
    if (props.scalesToZero ?? false) {
      (cluster.node.defaultChild as CfnResource).addPropertyOverride(
    ...
    if (props.createBastion) {
      ...
      new CfnOutput(this, "PortForwardCommand", {
        ...
        }"], "localPortNumber":["${cluster.clusterEndpoint.port}"], "host": ["${
  ...
  private runQuery(sql: string, database: string | undefined) {
    ...
    if (this.queries.length > 0) {
      ...
      query.node.defaultChild!.node.addDependency(
        this.queries.at(-1)!.node.defaultChild!
      );
    } else {
      ...
      const dbInstance = this.cluster.node.findChild(this.writerId).node
        .defaultChild!;
