...
export class ApiService extends Construct {
  ...
  constructor(scope: Construct, id: string, props: ApiServiceProps) {
    ...
    //Main コンテナ
    // イメージは ECR レジストリから取得 (langgenius/dify-api:${props.imageTag})
    //自社でイメージコンテも管理する場合はここは自社のECRのリポジトリを指定
    taskDefinition.addContainer("Main", {
      image: ecs.ContainerImage.fromRegistry(
        `langgenius/dify-api:${props.imageTag}`
      ),
      ...
      secrets: {
        ...
        PGVECTOR_USER: ecs.Secret.fromSecretsManager(
          postgres.secret,
          "username"
        ),
        ...
        PGVECTOR_PASSWORD: ecs.Secret.fromSecretsManager(
          postgres.secret,
          "password"
        ),
    ...
    taskDefinition.addContainer("ExternalKnowledgeBaseAPI", {
      image: ecs.ContainerImage.fromAsset(
        join(__dirname, "docker", "external-knowledge-api"),
        {
          platform: Platform.LINUX_AMD64,
          buildArgs: {
            DIFY_VERSION: props.sandboxImageTag,
    ...
    alb.addEcsService("Api", service, port, "/health", [
      ...paths,
      ...paths.map((p) => `${p}/*`),
    ]);
  }
